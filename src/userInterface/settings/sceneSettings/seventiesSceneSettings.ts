import type { SeventiesSceneSetting } from '@/src/scene/scenes/seventiesScene/setting';
...
export function seventiesSceneSettings(
    ...
    spawnFolder
        .add(settings, 'spawnThreshold', 0, 2000000)
    ...
    spawnFolder
        .add(settings, 'crowdSuppression', 0.0, 0.01)
    ...
    spawnFolder
        .add(settings, 'circleResolution', 1, 50)
        .step(1)
        .name('Circles Per Spawn')
        .onChange((value: number) => {
            settings.circleResolution = value;
            setSceneSettings(settings, sceneName, isExternalUi);
        });
    spawnFolder
        .add(settings, 'targetSize', 10, 200)
        .name('Target Size')
        .onChange((value: number) => {
    ...
    movementFolder
        .add(settings, 'fadeSpeed', 0.0, 2.0)
        .name('Fade Speed')
        .onChange((value: number) => {
    ...
    movementFolder
        .add(settings, 'speedMusicScale', 1, 50)
    ...
    movementFolder
        .add(settings, 'speedReducer', 100, 20000)
        .name('Speed Reducer')
        .onChange((value: number) => {
    ...
    movementFolder
        .add(settings, 'expansionSpeed', 0.0, 3.0)
    ...
    movementFolder
        .add(settings, 'hueRotationSpeed', 0.0, 5.0)
    ...
    movementFolder
        .add(settings, 'volumeSensitivity', 0.5, 5.0)
    ...
    appearanceFolder
        .add(settings, 'fillOpacity', 0.0, 1.0)
        .name('Fill Opacity')
        .onChange((value: number) => {
    ...
    creationFolder
        .add(settings, 'initialRadiusStep', 0.5, 10)
        .name('Initial Radius Step')
        .onChange((value: number) => {
            settings.initialRadiusStep = value;
            setSceneSettings(settings, sceneName, isExternalUi);
        });
    creationFolder
        .add(settings, 'targetSizeVariation', 0, 50)
        .name('Target Size Variation')
        .onChange((value: number) => {
            settings.targetSizeVariation = value;
            setSceneSettings(settings, sceneName, isExternalUi);
        });
    creationFolder
        .add(settings, 'hueStep', 0, 50)
        .name('Hue Step Between Circles')
        .onChange((value: number) => {
            settings.hueStep = value;
            setSceneSettings(settings, sceneName, isExternalUi);
        });
    creationFolder
        .add(settings, 'hueVariation', 0, 100)
        .name('Hue Variation')
        .onChange((value: number) => {
    ...
    creationFolder
        .add(settings, 'spreadAmount', 0, 100)
    ...
    creationFolder
        .add(settings, 'opacitySteps', 5, 50)
        .step(1)
        .name('Opacity Steps')
        .onChange((value: number) => {
