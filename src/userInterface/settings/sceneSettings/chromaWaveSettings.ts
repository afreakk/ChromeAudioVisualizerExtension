import type { ChromaWaveSetting } from '@/src/scene/scenes/chromaWave/setting';
...
export function chromaWaveSettings(
    ...
    settingsFolder
        .add(settings, 'audioSensitivity', 0.5, 5)
    ...
    settingsFolder
        .add(settings, 'lowSpeed', 0.01, 0.2)
        .name('Bass Speed')
        .onChange((value: number) => {
    ...
    settingsFolder
        .add(settings, 'midSpeed', 0.01, 0.2)
        .name('Mid Speed')
        .onChange((value: number) => {
    ...
    settingsFolder
        .add(settings, 'highSpeed', 0.01, 0.2)
        .name('Treble Speed')
        .onChange((value: number) => {
    ...
    // Base movement
    settingsFolder
        .add(settings, 'baseLowSpeed', 0, 0.1)
        .name('Base Bass Speed')
        .onChange((value: number) => {
            settings.baseLowSpeed = value;
            setSceneSettings(settings, sceneName, isExternalUi);
        });
    settingsFolder
        .add(settings, 'baseMidSpeed', 0, 0.1)
        .name('Base Mid Speed')
        .onChange((value: number) => {
    ...
    settingsFolder
        .add(settings, 'baseHighSpeed', 0, 0.1)
        .name('Base Treble Speed')
        .onChange((value: number) => {
    ...
    settingsFolder
        .add(settings, 'waveFrequency', 1, 10)
        .name('Wave Frequency')
        .onChange((value: number) => {
    ...
    // Effect style
    settingsFolder
        .add(settings, 'patternStyle', { Waves: 0, Spirals: 1, Plasma: 2 })
        .name('Pattern Style')
        .onChange((value: number) => {
