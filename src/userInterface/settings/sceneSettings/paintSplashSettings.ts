...
export function paintSplashSettings(
    ...
    settingsFolder.add(settings, 'numSplashes', 6, 64).step(1).name('Number of Splashes').onChange((value: number) => {
        settings.numSplashes = value;
        setSceneSettings(settings, sceneName, isExternalUi);
    });
    settingsFolder.add(settings, 'baseSize', 0.005, 0.1).name('Base Size').onChange((value: number) => {
        settings.baseSize = value;
        setSceneSettings(settings, sceneName, isExternalUi);
    });
    settingsFolder.add(settings, 'maxSize', 0.05, 0.3).name('Max Size').onChange((value: number) => {
        settings.maxSize = value;
        setSceneSettings(settings, sceneName, isExternalUi);
    });

    // Movement
    settingsFolder.add(settings, 'spreadRadius', 0.1, 0.8).name('Spread Radius').onChange((value: number) => {
    ...
    // Colors
    settingsFolder.add(settings, 'colorSpeed', 0, 2).name('Color Cycle Speed').onChange((value: number) => {
    ...
    // Effects
    settingsFolder.add(settings, 'fadeAmount', 0.01, 0.2).name('Trail Fade').onChange((value: number) => {
    ...
    settingsFolder.add(settings, 'trailPersistence').name('Trail Persistence').onChange((value: boolean) => {
