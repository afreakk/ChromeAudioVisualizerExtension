...
export function roundSpectrumSettings(
    ...
    spectrumFolder.add(settings, 'musicColorInfluenceReducer', 0.1, 1000).name('Audio Color Influence').onChange((value: number) => {
        settings.musicColorInfluenceReducer = value;
        setSceneSettings(settings, sceneName, isExternalUi);
    });
    spectrumFolder.add(settings, 'musicHeightPower', 0.0, 0.1).name('Audio Height Response').onChange((value: number) => {
        settings.musicHeightPower = value;
        setSceneSettings(settings, sceneName, isExternalUi);
    });
    spectrumFolder.add(settings, 'heightMultiplier', 0.0, 2.0).name('Height Multiplier').onChange((value: number) => {
    ...
    perfFolder.add(settings, 'maxBars', 64, 512).step(1).name('Max Bars').onChange((value: number) => {
        settings.maxBars = value;
        setSceneSettings(settings, sceneName, isExternalUi);
    });
    perfFolder.add(settings, 'barSkip', 1, 8).step(1).name('Bar Skip (Lower = More Bars)').onChange((value: number) => {
    ...
    geometryFolder.add(settings, 'innerRadiusDivisor', 5, 50).name('Inner Radius Size').onChange((value: number) => {
        settings.innerRadiusDivisor = value;
        setSceneSettings(settings, sceneName, isExternalUi);
    });
    geometryFolder.add(settings, 'staticWidth', 0.5, 3.0).name('Base Bar Width').onChange((value: number) => {
    ...
    effectsFolder.add(settings, 'glowThreshold', 0, 255).name('Glow Threshold').onChange((value: number) => {
