...
- [ ] **Cap CosmicAurora `pulseRings` and `shootingStars` arrays** — `src/scene/scenes/cosmicAurora/cosmicAurora.ts:309,633` — Both grow on beats with no hard cap. Only `auroraParticles` has `MAX_AURORA_PARTICLES`. Add hard caps. The `.filter()` calls also allocate new arrays each frame — consider in-place removal.
